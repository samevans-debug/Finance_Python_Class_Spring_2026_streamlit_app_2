...
weight restriction, which is user adjustable). The Optimal portfolios are calculated using 
the scipy.optimize.minimize() function with a bound (constraint: must sum to 1). Note, 
to use scipy.optimize.minimize() to maximize I just minimized the negative of the values and then reversed the sign.''')
...
approaches to calculating value at risk (VaR)—parametric, historical,
